...
/**
 * Spec 0004 B4 : PUT /api/backoffice/about/site-cards/order. Même grille que
 ...
 */
final class BackofficeAboutSiteCardOrderResourceTest extends WebTestCase
{
    use HttpJson;

    private const string ORDER_PATH = '/api/backoffice/about/site-cards/order';
    ...
    public function testReorderingIsReflectedOnTheBackofficeCollectionAndBothPublicEndpoints(): void
    {
        ...
        self::assertSame(['Second', 'Premier'], array_column($payload['site']['cards'], 'title'));

        $client->request('GET', '/api/about/en');
        self::assertResponseIsSuccessful();
        $payload = json_decode((string) $client->getResponse()->getContent(), true, flags: \JSON_THROW_ON_ERROR);
        self::assertSame(['Second, in English', 'First'], array_column($payload['site']['cards'], 'title'));
    }
    ...
    /**
     * Deux cartes « site », chacune en FR et en EN, plus les réglages des deux
     * locales sans lesquels la page publique répondrait 404.
     *
    ...
    {
        // Les deux endpoints publics /api/about/{locale} répondent 404 sans
        // réglages pour la locale demandée : ils en portent les intitulés.
        ...
        $first = $administrator->create(Locale::FR, 'Premier', 'description', 'boxes');
        $second = $administrator->create(Locale::FR, 'Second', 'description', 'columns-3');
        $administrator->create(Locale::EN, 'First', 'description', 'boxes', $first->getTranslationGroup());
        $administrator->create(Locale::EN, 'Second, in English', 'description', 'columns-3', $second->getTranslationGroup());
