...
/**
 * Spec 0004 B4 : PUT /api/backoffice/contributions/order. Même grille que
 ...
 */
final class BackofficeContributionOrderResourceTest extends WebTestCase
{
    use HttpJson;

    private const string ORDER_PATH = '/api/backoffice/contributions/order';
    ...
    /**
     * Deux contributions, chacune en FR et en EN : la seule forme qui permette
    ...
    private function seedTwoTranslatedEntries(KernelBrowser $client): array
    {
        ...
        $first = $administrator->create(Locale::FR, 'Premier', 'projet', 'PR #1', 'https://example.test/1', 'résumé', 'corps');
        $second = $administrator->create(Locale::FR, 'Second', 'projet', 'PR #2', 'https://example.test/2', 'résumé', 'corps');
        $administrator->create(Locale::EN, 'First', 'project', 'PR #1', 'https://example.test/1', 'summary', 'body', $first->getTranslationGroup());
        $administrator->create(Locale::EN, 'Second, in English', 'project', 'PR #2', 'https://example.test/2', 'summary', 'body', $second->getTranslationGroup());
