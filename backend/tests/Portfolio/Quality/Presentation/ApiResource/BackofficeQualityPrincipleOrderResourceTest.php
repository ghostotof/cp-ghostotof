...
/**
 * Spec 0004 B4 : PUT /api/backoffice/quality/principles/order. Même grille que
 ...
 */
final class BackofficeQualityPrincipleOrderResourceTest extends WebTestCase
{
    use HttpJson;

    private const string ORDER_PATH = '/api/backoffice/quality/principles/order';
    ...
    public function testReorderingIsReflectedOnTheBackofficeCollectionAndBothPublicEndpoints(): void
    {
        ...
        self::assertSame(['Second', 'Premier'], array_column($payload['principles'], 'title'));

        $client->request('GET', '/api/quality/en');
        self::assertResponseIsSuccessful();
        $payload = json_decode((string) $client->getResponse()->getContent(), true, flags: \JSON_THROW_ON_ERROR);
        self::assertSame(['Second, in English', 'First'], array_column($payload['principles'], 'title'));
    }
    ...
    /**
     * Deux principes qualité, chacun en FR et en EN.
     *
    ...
    private function seedTwoTranslatedEntries(KernelBrowser $client): array
    {
        ...
        $first = $administrator->create(Locale::FR, 'Premier', 'description', 'columns-3');
        $second = $administrator->create(Locale::FR, 'Second', 'description', 'boxes');
        $administrator->create(Locale::EN, 'First', 'description', 'columns-3', $first->getTranslationGroup());
        $administrator->create(Locale::EN, 'Second, in English', 'description', 'boxes', $second->getTranslationGroup());
