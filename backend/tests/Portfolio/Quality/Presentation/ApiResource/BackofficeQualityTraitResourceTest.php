...
final class BackofficeQualityTraitResourceTest extends WebTestCase
{
    ...
    public function testGetCollectionFiltersByLocaleQueryParameter(): void
    {
        ...
        $administrator->create(Locale::FR, 'Architecture propre');
        $administrator->create(Locale::EN, 'Clean architecture');
    ...
    public function testPostWithoutTranslationGroupGoesToTheEndOfTheScope(): void
    {
        ...
        $existing = $this->administrator($client)->create(Locale::FR, 'Premier');
        self::assertSame(0, $existing->getPosition());

        $created = $this->post($client, $csrfToken, ['label' => 'Second']);
    ...
    public function testPostWithAMalformedTranslationGroupIsRejected(): void
    {
        ...
        $this->post($client, $csrfToken, ['label' => 'x', 'translationGroup' => 'pas-un-uuid']);
    ...
    public function testAPositionInTheBodyIsIgnored(): void
    {
        ...
        $created = $this->post($client, $csrfToken, ['label' => 'Position imposée', 'position' => 99]);
    ...
    public function testPutWithANullTranslationGroupDetachesAndMovesToTheEnd(): void
    {
        ...
        $administrator = $this->administrator($client);
        $administrator->create(Locale::FR, 'Premier');
        $french = $administrator->create(Locale::FR, 'Second');
        $english = $administrator->create(Locale::EN, 'Second, in English', $french->getTranslationGroup());

        $updated = $this->put($client, $csrfToken, $french->getId()->toRfc4122(), [
            'label' => 'Second, détaché',
        ...
        $reloaded = self::getContainer()->get(QualityTraitRepositoryInterface::class)->findOneById($english->getId());
    ...
    public function testPutWithAnotherGroupAttachesAndInheritsItsPosition(): void
    {
        ...
        $administrator = $this->administrator($client);
        $target = $administrator->create(Locale::FR, 'Premier');
        $english = $administrator->create(Locale::EN, 'Orpheline');
        self::assertSame(1, $english->getPosition());

        $updated = $this->put($client, $csrfToken, $english->getId()->toRfc4122(), [
            'locale' => 'en',
            'label' => 'Premier, in English',
    ...
    public function testPutTowardsAGroupThatAlreadyCarriesTheLocaleIsAConflict(): void
    {
        ...
        $administrator = $this->administrator($client);
        $target = $administrator->create(Locale::FR, 'Premier');
        $other = $administrator->create(Locale::FR, 'Second');
    ...
    private function administrator(KernelBrowser $client): QualityTraitAdministratorInterface
    {
        return $client->getContainer()->get(QualityTraitAdministratorInterface::class);
    }
