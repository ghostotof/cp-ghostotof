...
- [x] 3.2b Actions d'administration journalisées avec l'auteur : invitation/réinvitation, rôle, mot de passe, suppression, activation — tests
- [x] 3.3 Requête `kubectl logs … | jq` documentée dans `k8s/README.md` (section « Journal de sécurité ») ; la vérification sur un pod réel se fait au CHECKPOINT 3, à la prochaine release
- [ ] 3.4 (suivi, hors D5 initial) événement `token-rejected` pour un JWT refusé sur le firewall `api` — à décider ; le canal `security` de Symfony trace déjà l'échec de l'authenticator
