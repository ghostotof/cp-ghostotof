...
### Checkpoint B3 — `v0.12.0` en production — atteint le 2026-09-14
- [x] Pile #155 → #168 mergée dans `develop` dans l'ordre, avec retargeting avant suppression de branche ; `develop` → `main`
- [x] `DEPLOY_MAINTENANCE_WINDOW=true` posée **avant** le tag (le rollout standard précède la migration, cf. spec 0004 §10), retirée après la prod
- [x] Tag `v0.12.0 --cleanup=verbatim` avec les notes ; préprod : migration jouée, appariement vérifié sur les données seedées (un groupe par contenu bilingue), un réordonnancement relu sur `/api/<x>/fr` **et** `/api/<x>/en`
- [x] Prod : migration jouée, appariement vérifié par SQL (tous bilingues, 0 orphelin, positions homogènes), pages publiques inchangées ; release publiée par `create-release` (run 34902547024) — [#152](https://github.com/ghostotof/cp-ghostotof/issues/152)
