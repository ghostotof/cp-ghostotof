...
| **Durée de conservation** | Tant que l'accès est accordé : le compte et son adresse sont supprimés par l'éditeur depuis le backoffice, à la fin du besoin ou sur simple demande de la personne (§7) ; la suppression emporte ses jetons (`ON DELETE CASCADE`). Jeton : 48 h, remplacé à chaque renvoi d'invitation. **Invitation jamais activée : 30 jours après la dernière invitation**, puis suppression automatique du compte et de ses jetons par `app:user:purge-pending-invitations` (CronJob de maintenance quotidien, 03:17, `k8s/base/messenger-purge-cronjob.yaml`), tracée dans le journal d'audit (§6, événement `user-purged`, acteur `system`, jamais l'adresse). « Jamais activée » se vérifie sur le mot de passe, pas seulement sur l'absence de date d'activation : le prédicat exact est un compte dont le hachage de mot de passe est encore vide (`CpgUser::isAwaitingPasswordSetup()`) — si un `ROLE_SUPER` a posé un mot de passe pour la personne depuis le backoffice sans que celle-ci soit passée par le lien reçu, le compte se connecte déjà et sort donc de la purge. Un compte en attente qui porterait `ROLE_SUPER` n'est pas purgé automatiquement : sa suppression reste une décision manuelle. Renvoyer l'invitation depuis le backoffice repousse le délai (relance = nouvelle invitation au sens de ce délai). La durée de rétention n'est jamais inférieure à un jour, y compris pour un lancement manuel de la commande. |
...
| **Données collectées** | Par enregistrement : `event` (clé stable de l'événement), `actor` (identifiant du jeton de sécurité, `anonymous`, ou `system` pour une action automatique sans intervention humaine — la purge d'invitation ci-dessous), `ip` (adresse IP de l'appelant), `path` (chemin de la requête, forme décodée), l'horodatage et le canal ajoutés par Monolog, et selon l'événement `user` (le **nom d'utilisateur** saisi ou visé, ou l'identifiant éphémère `guest-…` d'un jeton de palier de base), `userId` (UUID du compte), `superAdmin` (booléen) et `reason` (motif stable de l'action automatique, aujourd'hui la seule valeur `invitation-expired`, purge d'une invitation jamais activée — issue #238) |
...
| **Arbitrage** | Après suppression d'un compte (manuelle ou par la purge automatique d'invitation, §3.2), le journal continue de désigner le compte disparu par son nom d'utilisateur (dérivé de la partie locale de l'adresse email, `UsernameGenerator`) et son UUID : c'est une tension assumée entre traçabilité (art. 32 RGPD, savoir qui a fait quoi) et minimisation (art. 5-1-c, ne garder que le nécessaire). Choix retenu : la conserver, parce qu'elle reste bornée par la durée de conservation des journaux eux-mêmes (§5, journaux de conteneur sans export ni archivage) et qu'elle ne réintroduit aucune adresse email |
