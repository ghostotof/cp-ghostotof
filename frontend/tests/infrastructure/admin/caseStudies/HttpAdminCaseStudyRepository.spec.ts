...
import { HttpAdminCaseStudyRepository } from '../../../../src/infrastructure/admin/caseStudies/HttpAdminCaseStudyRepository'
...
const BASE_PATH = `${API_BASE_URL}/api/backoffice/case-studies`

const API_SECTION = {
  id: SECTION_ID, locale: 'fr', translationGroup: GROUP_ONE, title: 'Un moteur de tarification',
...
describe('HttpAdminCaseStudyRepository', () => {
  beforeEach(() => {
  ...
  it('list() appelle GET avec credentials include et sans header CSRF', async () => {
    ...
    const caseStudies = await new HttpAdminCaseStudyRepository(API_BASE_URL).list()

    expect(fetchMock).toHaveBeenCalledWith(BASE_PATH, { method: 'GET', credentials: 'include' })
    expect(caseStudies).toEqual([API_SECTION])
  })
  ...
  it('create() envoie POST avec le corps JSON, groupe de traduction compris et sans position', async () => {
    ...
    await new HttpAdminCaseStudyRepository(API_BASE_URL).create({ ...INPUT, translationGroup: GROUP_TWO })
  ...
  it("update() cible l'id dans le chemin", async () => {
    ...
    await new HttpAdminCaseStudyRepository(API_BASE_URL).update(TARGET_SECTION_ID, INPUT)
  ...
  it("remove() envoie DELETE sur l'id", async () => {
    ...
    await new HttpAdminCaseStudyRepository(API_BASE_URL).remove(TARGET_SECTION_ID)
  ...
  it('reorder() envoie PUT …/order avec la liste complète des groupes', async () => {
    ...
    await new HttpAdminCaseStudyRepository(API_BASE_URL).reorder([GROUP_TWO, GROUP_ONE])
  ...
  it.each([
    ...
    const error = await new HttpAdminCaseStudyRepository(API_BASE_URL).reorder([GROUP_ONE]).catch((c: unknown) => c)
  ...
  it('traduit un 404 en erreur de domaine « not-found »', async () => {
    ...
    const error = await new HttpAdminCaseStudyRepository(API_BASE_URL)
      .update(MISSING_SECTION_ID, INPUT)
      .catch((c: unknown) => c)

    expect(error).toBeInstanceOf(AdminCaseStudyError)
    expect((error as AdminCaseStudyError).reason).toBe('not-found')
  })
  ...
  it('traduit un 409 en erreur de domaine « translation-already-exists »', async () => {
    ...
    expect(error).toBeInstanceOf(AdminCaseStudyError)
    expect((error as AdminCaseStudyError).reason).toBe('translation-already-exists')
  })
  ...
  it('traduit un 422 /errors/unknown-translation-group en son propre motif, distinct de la validation', async () => {
    ...
    expect((error as AdminCaseStudyError).reason).toBe('unknown-translation-group')
  })

  it('traduit un 422 en erreur de domaine « validation » — résultat mesuré vide, par exemple', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => jsonResponse(422, { violations: [{ propertyPath: 'measuredResult', message: 'Cette valeur ne doit pas être vide.' }] })),
    )

    const error = await new HttpAdminCaseStudyRepository(API_BASE_URL)
      .create({ ...INPUT, measuredResult: '' })
      .catch((c: unknown) => c)

    expect(error).toBeInstanceOf(AdminCaseStudyError)
    expect((error as AdminCaseStudyError).reason).toBe('validation')
    expect((error as AdminCaseStudyError).message).toBe('Cette valeur ne doit pas être vide.')
  })
})
