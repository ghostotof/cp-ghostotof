...
const { legalNoticeContent } = usePortfolioContent()
</script>

<template>
  <section class="container-xl py-5 d-flex flex-column gap-4">
    <div class="surface-panel p-3 p-sm-4">
      <p class="d-flex align-items-center gap-2 text-eyebrow text-uppercase small fw-semibold mb-3">
        <span
          class="rounded-circle bg-primary"
          style="width: 0.4rem; height: 0.4rem"
          aria-hidden="true"
        />
        {{ legalNoticeContent.eyebrow }}
      </p>
      <h1 class="h3 fw-bold text-white mb-1">
        {{ legalNoticeContent.title }}
      </h1>
      <p class="text-body-secondary small mb-4">
        {{ legalNoticeContent.lastUpdated }}
      </p>

      <div
        v-for="(section, index) in legalNoticeContent.sections"
        :key="section.heading"
        :class="{ 'border-top mt-4 pt-4': index > 0 }"
      >
