...
<template>
  <section id="technologies" class="container-xl py-4">
    <div class="surface-panel p-3 p-sm-4">
      <p class="d-flex align-items-center gap-2 text-primary text-uppercase small fw-semibold mb-4">
        <span class="rounded-circle bg-primary" style="width: 0.4rem; height: 0.4rem" />
        Technologies
      </p>

      <div class="row row-cols-2 row-cols-sm-3 row-cols-lg-6 g-3">
        <div v-for="technology in featuredTechnologies" :key="technology.name" class="col">
          <BaseCard
      ...
      >
        <span class="fw-semibold text-white">ET AUSSI</span>
        <template v-for="(technology, index) in additionalTechnologies" :key="technology.name">
          <span v-if="index > 0" class="text-body-secondary">&middot;</span>
