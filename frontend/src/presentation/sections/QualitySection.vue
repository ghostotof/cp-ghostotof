...
defineProps<{
  qualityPrinciples: readonly QualityPrinciple[]
  qualityTraits: readonly QualityTrait[]
}>()
  ...
  <section class="container-xl py-4">
    <div class="surface-panel p-3 p-sm-4">
      ...
      <div class="row row-cols-1 row-cols-sm-3 g-3">
        <div
          v-for="principle in qualityPrinciples"
          :key="principle.title"
          class="col"
        >
          <BaseCard
            ...
            :icon-key="principle.iconKey"
          />
        </div>
      </div>

      <div class="d-flex flex-wrap gap-3 border-top mt-4 pt-4">
        <BaseBadge
          v-for="trait in qualityTraits"
          :key="trait.label"
          icon-key="check"
        >
          {{ trait.label }}
